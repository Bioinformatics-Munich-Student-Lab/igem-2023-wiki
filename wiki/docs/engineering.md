...
## The design cycle in machine learnign development

Building an AI model typically involves a structured process consisting of four key steps: design, build, test, and learn. Here's an overview of each step:

### Design

The first step is to clearly define the problem you want the AI model to solve.\
...
Consequently, reimplement `Decagon` was our first deliverable.\
...
Consequently, we were getting adequate results for pairwise predictions, but had not yet fufilled our goal of enabling true polypharmacy predictions.
...
Based on what we were able to learn from the previous cycle, we made the decision to rebuild the entire architecture.
...
We implemented our model [PyTorch Geometric](https://pytorch-geometric.readthedocs.io/en/latest/index.html) using the [SAGEConv](https://arxiv.org/pdf/1706.02216.pdf) architecture.\
...
This would lead to memory issues late on.
...
We found that the the full training graph constructed to train the model would incorporate all of the information required, but would crash during the training process because it was using more memory than our computers had available.
...
We were able to observe an increase in the quality of predicions.
...
This is simpler to predict, and training the main model to differentiate these fake interactions from real interactions has shown to improve training performance, particularly in graph neural networks [^7].
